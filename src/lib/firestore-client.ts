...
export async function saveDateOverride(
  ...
  await setDoc(ref, { closed: override.closed ?? false, slots: cleanedSlots });
}
...
export async function fetchAvailability(): Promise<DayAvailability[]> {
  ...
  scheduleSnap.forEach((docSnap) => {
    ...
    const data = docSnap.data() as { slots?: Slot[]; times?: string[]; locations?: string[] };
  ...
  overridesSnap.forEach((docSnap) => {
    const data = docSnap.data() as { closed?: boolean; slots?: Slot[] };
    const slots = (data.slots || [])
      .map((slot) => ({ time: String(slot?.time ?? "").trim(), location: String(slot?.location ?? "").trim() }))
      .filter((slot) => slot.time && slot.location);
    overridesByDate.set(docSnap.id, { closed: data.closed ?? false, slots });
  });

  const bookingsSnap = await getDocs(
    query(collection(db, "bookings"), where("date", ">=", todayStr), where("status", "==", "confirmed"))
  );

  const bookingsByDate = new Map<string, Array<{ time: string; location: string }>>();

  bookingsSnap.forEach((docSnap) => {
    const data = docSnap.data() as { date?: string; time?: string; location?: string };
  ...
  for (let i = 2; i < daysAhead; i++) {
    ...
    const uniqueTimes = [...new Set(daySlots.map((slot) => slot.time).filter(Boolean))].sort(sortTimes);

    for (const time of uniqueTimes) {
      if (dateStr === todayStr && timeToMinutes(time) <= nowMinutes) continue;

      const scheduledLocations = [...new Set(daySlots.filter((slot) => slot.time === time).map((slot) => slot.location).filter(Boolean))];
      const exactBookedLocations = new Set(dayBookings.filter((b) => b.time === time).map((b) => b.location));

      let availableLocations = scheduledLocations.filter((loc) => !exactBookedLocations.has(loc));

      const nearbyBookings = dayBookings.filter((b) => Math.abs(timeToMinutes(b.time) - timeToMinutes(time)) <= 120);

      if (nearbyBookings.length > 0) {
        const lockedLocations = new Set(nearbyBookings.map((b) => b.location));
        availableLocations = availableLocations.filter((loc) => lockedLocations.has(loc));
      }

      availableLocations = [...new Set(availableLocations)].sort();
      if (availableLocations.length > 0) timeMap.set(time, availableLocations);
    }

    const timeSlots = Array.from(timeMap.entries()).map(([time, locations]) => ({ time, locations }));
    if (timeSlots.length === 0) continue;

    result.push({ id: dateStr, date: dateStr, timeSlots });
  }
...
export async function createBooking(
  ...
  const { date, time, location, name, email, phone, experience, dojo, message } = input;
  ...
  if (overrideDoc.exists()) {
    ...
    const weeklyData = weeklyDoc.data() as { slots?: Slot[]; times?: string[]; locations?: string[] };
    validSlots = normalizeWeeklySlots(weeklyData);
  }

  const scheduledForThisTime = validSlots.filter((slot) => slot.time === time).map((slot) => slot.location);
  ...
  const bookingsSnap = await getDocs(
    query(collection(db, "bookings"), where("date", "==", date), where("status", "==", "confirmed"))
  );

  const dayBookings = bookingsSnap.docs.map((docSnap) => {
    const data = docSnap.data() as { time?: string; location?: string };
    return { time: String(data.time ?? "").trim(), location: String(data.location ?? "").trim() };
  });

  // 3. Check exact duplicate
  if (dayBookings.some((b) => b.time === time && b.location === location)) {
  ...
  if (nearbyBookings.length > 0) {
    ...
    if (!lockedLocations.has(location)) {
      return { success: false, error: "This location is not available within 2 hours of another booking" };
    }
  }
