...
export default function AdminPage() {
  ...
  const uniqueLocationCount = useMemo(() => {
    return new Set(
      currentDay.slots.map((slot) => slot.location).filter(Boolean)
    ).size;
  }, [currentDay]);

  // Admin: freely toggle any location for any time (no 2h restriction here)
  const toggleLocationForTime = (
    day: number,
    time: string,
    location: string
  ) => {
    ...
    const exists = current.slots.some(
      (slot) => slot.time === time && slot.location === location
    );

    let nextSlots: Slot[];

    if (exists) {
      nextSlots = current.slots.filter(
        (slot) => !(slot.time === time && slot.location === location)
      );
    } else {
  ...
  const s = {
    ...
    tab: (active: boolean) =>
      ({
        ...
        color: active ? "#38bdf8" : "#e8e4df",
        padding: "10px 24px",
        cursor: "pointer",
        fontSize: 14,
        fontWeight: 500,
      }) as React.CSSProperties,
    dayTab: (active: boolean, hasSchedule: boolean) =>
      ({
        padding: "10px 16px",
        cursor: "pointer",
        fontSize: 13,
    ...
    <div style={s.page}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 32 }}>
          <button
            style={s.tab(tab === "schedule")}
            onClick={() => setTab("schedule")}
          >
            Weekly Schedule
          </button>
          <button
            style={s.tab(tab === "bookings")}
        ...
        {tab === "schedule" && (
          <div style={s.card}>
            ...
            >
              Set your weekly availability. You can choose multiple locations per
              time slot. The 2-hour location lock applies only to the booking
              page.
            </p>
            ...
            >
              {DAY_NAMES.map((name, i) => {
                const hasSchedule =
                  (schedule[String(i)]?.slots?.length ?? 0) > 0;
                return (
                  <div
                    key={i}
                    onClick={() => setSelectedDay(i)}
                    style={s.dayTab(selectedDay === i, hasSchedule)}
                  >
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button style={s.btnSecondary} onClick={copyToWeekdays}>
                    Copy to weekdays
                  </button>
                  <button style={s.btnSecondary} onClick={copyToAllDays}>
                    Copy to all days
                  </button>
                  <button
                    style={s.btnDanger}
              ...
              <div style={{ display: "grid", gap: 18 }}>
                {TIME_OPTIONS.map((time) => {
                  const selectedLocations = getLocationsForTime(
                    currentDay.slots,
                    time
                  );
                    ...
                    >
                      ...
                      >
                        ...
                        {selectedLocations.length > 0 && (
                          <button
                            style={s.btnDanger}
                            onClick={() => clearTime(selectedDay, time)}
                          >
                            Clear time
                          </button>
                        )}
            ...
            >
              <label style={s.label}>Selected Day Summary</label>
              <div
                style={{
                  fontSize: 14,
              ...
              >
                {currentDay.slots.length} total slot
                {currentDay.slots.length === 1 ? "" : "s"} ·{" "}
                {uniqueLocationCount} location
                {uniqueLocationCount === 1 ? "" : "s"}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {slotCount > 0 ? (
                        ...
                        <div style={{ color: "rgba(232,228,223,0.2)" }}>
                          OFF
                        </div>
                      )}
                    </div>
        ...
        {tab === "bookings" && (
          <div style={s.card}>
            ...
            {loading && (
              <p style={{ color: "rgba(232,228,223,0.5)" }}>Loading...</p>
            )}

            {bookingsList.length === 0 && !loading && (
              <p style={{ color: "rgba(232,228,223,0.4)" }}>
                No bookings yet.
              </p>
            )}
                        ...
                        style={{
                          ...
                          background:
                            b.status === "confirmed"
                              ...
                              : "rgba(239,68,68,0.15)",
                          color:
                            b.status === "confirmed" ? "#38bdf8" : "#ef4444",
                        }}
                      >
                        {b.status}
                      </span>
